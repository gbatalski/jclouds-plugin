...
public class ProvisionPlannedInstancesAndDestroyAllOnError implements
         Function<Iterable<NodePlan>, Iterable<RunningNode>> {
   private final ListeningExecutorService executor;
   private final Logger logger;
   private final Function<Iterable<RunningNode>, Void> terminateNodes;

   public ProvisionPlannedInstancesAndDestroyAllOnError(ListeningExecutorService executor, Logger logger,
   ...
   public Iterable<RunningNode> apply(Iterable<NodePlan> nodePlans) {
      ...
      final ImmutableList.Builder<ListenableFuture<NodeMetadata>> plannedInstancesBuilder = ImmutableList
               .<ListenableFuture<NodeMetadata>> builder();
      ...
      for (final NodePlan nodePlan : nodePlans) {
         for (int i = 0; i < nodePlan.getCount(); i++) {
            final int index = i;
            logger.info("Queuing cloud instance: #%d %d, %s %s", index, nodePlan.getCount(), nodePlan.getCloudName(),
            ...
            Futures.addCallback(provisionTemplate, new FutureCallback<NodeMetadata>() {
               public void onSuccess(NodeMetadata result) {
                  if (result != null) {
                     cloudTemplateNodeBuilder.add(new RunningNode(nodePlan.getCloudName(), nodePlan.getTemplateName(),
				    nodePlan.isSuspendOrTerminate(), result, nodePlan.getLabelSet(), nodePlan.isRegisterAsSlave(),
				    nodePlan.isKeepAlive()));
                  } else {
               ...
               public void onFailure(Throwable t) {
                  failedLaunches.incrementAndGet();
                  logger.warn(t, "Error while launching instance: #%d %d, %s %s", index, nodePlan.getCount(), nodePlan
                           .getCloudName(), nodePlan.getTemplateName());
               }
            });
      ...
      List<NodeMetadata> nodesActuallyLaunched = Futures.getUnchecked(Futures.successfulAsList(plannedInstancesBuilder
               .build()));
      ...
      assert cloudTemplateNodes.size() == nodesActuallyLaunched.size() : String.format(
               "expected nodes from callbacks to be the same count as those from the list of futures!%n"
                        + "fromCallbacks:%s%nfromFutures%s%n", cloudTemplateNodes, nodesActuallyLaunched);
