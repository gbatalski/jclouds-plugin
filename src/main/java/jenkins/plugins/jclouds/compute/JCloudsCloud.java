...
import jenkins.plugins.jclouds.compute.elb.EnhancedELBRestClientModule;
import jenkins.plugins.jclouds.compute.elb.EnhancedLoadBalancerService;
import jenkins.plugins.jclouds.compute.elb.EnhancedLoadBalancerServiceContext;
import jenkins.plugins.jclouds.compute.elb.EnhancedLoadBalancerServiceModule;
...
public class JCloudsCloud extends Cloud {
    ...
    private transient EnhancedLoadBalancerService lb;
    ...
    public JCloudsCloud(final String profile, final String providerName, final String identity, final String credential,
	    final String privateKey, final String publicKey, final String endPointUrl, final int instanceCap,
	    final int retentionTime, final int scriptTimeout, final List<JCloudsSlaveTemplate> templates, final String loadbalancer) {
    ...
    static EnhancedLoadBalancerServiceContext lbCtx(String loadbalancer, String identity, String credential, Properties overrides) {
	// correct the classloader so that extensions can be found
	Thread.currentThread().setContextClassLoader(Apis.class.getClassLoader());
	return (EnhancedLoadBalancerServiceContext) ContextBuilder
		.newBuilder(loadbalancer)
		.credentials(identity, credential)
		.modules(
			Iterables.concat(MODULES,
				ImmutableList.of(new EnhancedELBRestClientModule(), new EnhancedLoadBalancerServiceModule())))
		.overrides(overrides).buildView(LoadBalancerServiceContext.class);
    ...
    public EnhancedLoadBalancerService getLb() {
	if (this.lb == null) {
	    Properties overrides = new Properties();
	...
	for (; excessWorkload > 0; excessWorkload--) {
		...
		public Node call() throws Exception {
		    ...
		    // Cloud instances may have a long init script. If we
		    // declare
   ...
   public JCloudsSlaveTemplate getTemplate(String name) {
    ...
    /**
     * Gets {@link jenkins.plugins.jclouds.compute.JCloudsSlaveTemplate} that
     * has the matching {@link Label}.
     */
     ...
     */
    public void doProvision(StaplerRequest req, StaplerResponse rsp, @QueryParameter String name) throws ServletException,
	...
	if (getRunningNodesCount() < instanceCap) {
	    ...
	    sendError("Instance cap for this cloud is now reached for cloud profile: " + profile + " for template type " + name,
		    req, rsp);
	}
    }
	...
	public FormValidation doTestConnection(@QueryParameter String providerName, @QueryParameter String identity,
		@QueryParameter String credential, @QueryParameter String privateKey, @QueryParameter String endPointUrl) {
	    if (identity == null)
		...
		result = FormValidation.error("Cannot connect to specified cloud, please check the identity and credentials: "
			+ ex.getMessage());
	    } finally {
	...
	public FormValidation doGenerateKeyPair(StaplerResponse rsp, String identity, String credential) throws IOException,
		ServletException {
	...
	public FormValidation doCheckPrivateKey(@QueryParameter String value) throws IOException, ServletException {
		...
		return FormValidation
			.error("Invalid private key, please check again or click on 'Generate Key' to generate a new key");
